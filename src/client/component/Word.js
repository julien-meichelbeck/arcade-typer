...
const charColor = ({ char, index, inputValue, isCurrentWord }) =>
...
export default recompact.pure(({ children: word, isCurrentWord, oponentColor, blurry, inputValue }) => {
  ...
  return (
    <span>
      <span style={{ textShadow, boxShadow }}>
        {chars.map((char, index) => (
          <Char key={index} char={char} color={charColor({ char, index, inputValue, isCurrentWord })} />
        ))}
      </span>{' '}
    </span>
  )
